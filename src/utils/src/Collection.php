...
use Larmias\Utils\Traits\Macroable;
use ArrayAccess;
use ArrayIterator;
use Countable;
...
class Collection implements ArrayAccess, Countable, IteratorAggregate, JsonSerializable, Arrayable, Jsonable
{
     ...
     * @return self
     */
    public function dictionary(mixed $items = null, ?string &$indexKey = null): self
    {
        ...
        if (isset($indexKey) && is_string($indexKey)) {
            return new static(array_column($items, null, $indexKey));
        }
     ...
     * @return static
     */
    public function diff(mixed $items, ?string $indexKey = null): self
    {
        ...
        $diff = [];
        $dictionary = $this->dictionary($items, $indexKey)->all();
     ...
     * @return static
     */
    public function intersect(mixed $items, string $indexKey = null): self
    {
        if ($this->isEmpty() || is_scalar($this->items[0])) {
     ...
     * @return $this
     */
    public function push(mixed $value, string $key = null): self
    {
        if (is_null($key)) {
            $this->items[] = $value;
        } else {
     ...
     * @return $this
     */
    public function unshift(mixed $value, string $key = null): self
    {
        if (is_null($key)) {
    ...
    public function contains(mixed $key, mixed $operator = null, mixed $value = null): bool
    {
        ...
        return $this->contains(function () use ($key, $operator, $value) {
            return $this->where($key, $operator, $value)->toArray();
        });
    }
     ...
     * @return static
     */
    public function whereBetween(string $field, mixed $value): self
    {
     ...
     * @return static
     */
    public function whereNotBetween(string $field, mixed $value): self
    {
     ...
     * @return self
     */
    public function column(?string $columnKey, string $indexKey = null): self
    {
